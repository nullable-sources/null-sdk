...
public:
    ...
    rect_t(corners_t _value) : rect_t{ _value, _value } { }
    rect_t(const vec2_t<corners_t>& _vec) : rect_t{ _vec, _vec } { }

    rect_t(corners_t _min, corners_t _max) : min{ _min }, max{ _max } { }
    rect_t(const vec2_t<corners_t>& _min, const vec2_t<corners_t>& _max) : min{ _min }, max{ _max } { }

    rect_t(corners_t a, corners_t size, const null::e_rect_origin& _origin) : min{ a } { origin(_origin, size); }
    rect_t(const vec2_t<corners_t>& a, const vec2_t<corners_t>& size, const null::e_rect_origin& _origin) : min{ a } { origin(_origin, size); }

    rect_t(corners_t min_x, corners_t min_y, corners_t max_x, corners_t max_y) : min{ min_x, min_y }, max{ max_x, max_y } { }
...
public:
    ...
    bool contains(const rect_t<corners_t>& rect) const { return contains(rect.min) || contains(rect.max); }
    bool intersects(const rect_t<corners_t>& rect) const {
    ...
    rect_t& scale(null::e_rect_origin _origin, const vec2_t<float>& _scale) { return scale(scale_from_origin(_origin), _scale); }
    ...
    vec2_t<corners_t> origin(null::e_rect_origin _origin) const { return origin(scale_from_origin(_origin)); }
    rect_t& origin(null::e_rect_origin _origin, const vec2_t<corners_t>& size) { return origin(scale_from_origin(_origin), size); }
...
public:
    ...
    auto&& operator [](this auto&& self, const int& i) { return self.corners[i]; }

    auto&& operator ++(this auto&& self) { ++self.min; ++self.max; return self; }
