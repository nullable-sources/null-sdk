...
namespace ntl {
    namespace win {
        ...
        vec2_t<int> c_window::get_window_size() const {
            ...
            return vec2_t<int>(wnd_rect.right - wnd_rect.left, wnd_rect.bottom - wnd_rect.top);
        }
        ...
        LRESULT WINAPI c_window::wnd_proc(HWND _wnd_handle, UINT msg, WPARAM w_param, LPARAM l_param) {
            if(c_window* window = (c_window*)GetWindowLongPtrA(_wnd_handle, 0)) {
                ...
                if(auto found = std::ranges::find_if(results, [](int result) { return result != -1; }); found != results.end())
                    return *found;
            }
    ...
    namespace console {
        ...
        bool i_command::handle(std::string_view str) {
            ...
            for(std::string_view command : str | std::views::split(';') | std::views::transform([](const auto& splitted) { return std::string_view{ splitted }; })) {
                ...
                if(auto found = std::ranges::find(i_command::registered_commands, args.front(), &i_command::name); found != i_command::registered_commands.end()) {
                    return (*found)->execute(args | std::views::drop(1) | std::ranges::to<std::vector>());
                }
            }
