        ...
        public:
            void process_event(const e_log_type& id, const std::unordered_map<std::string, std::any>& parameters) override {
                switch(id) {
                    ...
                    case e_log_type::error:     { std::cout << "\033[3;42;30m[error]\033[0m \x1B[31m"   << std::any_cast<std::string>(parameters.at("text")) << "\033[0m" << std::endl; } break;
                    case e_log_type::assert:    { throw std::runtime_error{ std::format("\033[3;42;30m[assert]\033[0m \x1B[31m{}\033[0m\n", std::any_cast<std::string>(parameters.at("text"))) }; } break;
                }
            }
        } default_listener{ };
